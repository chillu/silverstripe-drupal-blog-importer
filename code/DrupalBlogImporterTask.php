...
class DrupalBlogImporterTask extends BuildTask
{
    ...
    public function getCommentLoader()
    {
        if (!$this->commentLoader) {
            $this->commentLoader = Injector::inst()->create('DrupalBlogCommentBulkLoader');
        }
    ...
    {
        if (!$this->postLoader) {
            $this->postLoader = Injector::inst()->create('DrupalBlogPostBulkLoader');
        }
    ...
    {
        if (!$this->userLoader) {
            $this->userLoader = Injector::inst()->create('DrupalBlogUserBulkLoader');
        }
